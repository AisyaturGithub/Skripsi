...
with tab1:
    ...
    if periksa:
        ...
        if user_input == "":
            ...
            judul = data.tag_sents(result['']) 
            st.success(f"hasil correction:{judul}")
            # # Creating DataFrames
...
with tab2:
    ...
    if uploaded_file:
        ...
        def komputasi(row):
            ...
            return result, rata2recall, rata2precision
        #untuk melakukan perhitungan koreksi kata presisi dan recall pada data yg uji  
        hasil_koreksi = []
        recall = []
        precision = []
        for index, row in data.iterrows():
            result,rata2recall,rata2precision = komputasi(row)
        ...
        st.subheader("Hasil Precision dan Recall")
