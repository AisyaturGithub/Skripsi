    ...
    # Inisialisasi CRFTagger dilakukan di luar loop karena kita hanya perlu melakukannya sekali
        ...
        if periksa:
            ...
            if user_input == "":
                ...
                df_hasil = pd.DataFrame(hasil_itung)
                # menghitung presisi ejaan
                def presisi_ejaan(data_koreksi):
                    mycursor = mydb.cursor()
                    mycursor.execute(f"SELECT Unique_Words FROM data_kamus_pariwisata")
                    referensi = set(row[0] for row in mycursor.fetchall())
        
                    # Hitung jumlah koreksi yang benar dan jumlah semua koreksi yang dibuat
                    koreksi_benar = sum(1 for kata in data_koreksi if kata in referensi)
                    jumlah_koreksi = len(data_koreksi)
                    
                    # Hitung presisi
                    presisi = koreksi_benar / jumlah_koreksi if jumlah_koreksi > 0 else 0

                    #st.write(f"judul_list")
                    mycursor.close()
                    return presisi
                presisi_ejaan = presisi_ejaan(hasil1)
                st.info(f"Nilai Precision: {presisi_ejaan}")
...
#memanggil correction spelling dari levenshtein distance
        ...
        if uploaded_file: 
            ...
            # Fetch the word list once
                def fetch_word_list():
                    mycursor = mydb.cursor()
                    mycursor.execute("SELECT Unique_Words FROM kamus_berita_pariwisata__2_")
